...
function cleanProductTitle(value: string | null | undefined): string {
  return String(value ?? "Produto Tray")
...
function assertColumns(rows: CsvRow[], required: string[], label: string) {
  if (rows.length === 0) throw new Error(`O arquivo de ${label} está vazio.`);
...
export function buildTrayImportDataset(orderRows: CsvRow[], itemRows: CsvRow[]): TrayImportDataset {
  ...
  for (const row of orderRows) {
    ...
    const customerId = email ? `email:${email}` : trayCustomerCode ? `tray:customer:${trayCustomerCode}` : `tray:order-customer:${trayOrderCode}`;
    ...
    const fullName = clean(row["Nome do Cliente"]);
  ...
  for (const row of itemRows) {
    ...
    const quantity = Math.max(0, Math.trunc(Number(String(row["Quantidade"] ?? "0").replace(",", ".")) || 0));
  ...
  return {
    ...
    stats: {
      ...
      periodStart: orders[0]?.createdAt ?? null,
