...
as $$
  ...
  join_counts as (select landing_page_id, count(*) as joins from joined group by landing_page_id)
  select m.landing_page_id, m.visits, m.submissions, m.clicks, coalesce(j.joins, 0)
  from metrics m left join join_counts j using (landing_page_id);
$$;
...
create index if not exists fe_group_events_join_group_phone_created_at_idx
  on public.fe_group_events (group_id, phone, created_at)
  where event_type = 'join' and phone is not null;
