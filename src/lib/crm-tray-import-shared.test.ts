...
describe("crm-tray-import-shared", () => {
  ...
  it("une pedidos e itens pelo código do pedido sem duplicar o cliente", () => {
    ...
    expect(dataset.customers[0].id).toBe("email:maria@example.com");
    expect(dataset.customers[0].phone).toBe("+5531999998888");
    expect(dataset.orders[0].id).toBe("tray:100");
    expect(dataset.orders[0].financialStatus).toBe("PAID");
    ...
    expect(dataset.items[2].sku).toBe("TRAY-12");
