...
-- Nome sugerido: 20260824002500_create_whatsapp_message_queue.sql
-- Compatível com src/lib/whatsapp-queue.server.ts
...
-- Dedup: índice único PARCIAL (permite várias linhas com dedup_key NULL)
...
-- Sem policies e sem grants para anon/authenticated: acesso só via service_role.

CREATE TRIGGER whatsapp_message_queue_updated_at
  BEFORE UPDATE ON public.whatsapp_message_queue
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Claim atômico usado pelo worker
...
-- Requeue seguro de jobs travados em 'sending'
